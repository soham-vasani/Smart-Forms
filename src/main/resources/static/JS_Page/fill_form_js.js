...
function saveFilledForm(fId){
    ...
    if (!formIsValid) {
        ...
        if(Array.isArray(formResponse.answerResponse)){

            formResponse.answerResponse.forEach((eachAnswerResponse) => {

                
                eachAnswerResponse.optionId = eachAnswerResponse.optionId.filter(option => option != undefined && option != null);

                if(eachAnswerResponse.optionId.length > 0){
